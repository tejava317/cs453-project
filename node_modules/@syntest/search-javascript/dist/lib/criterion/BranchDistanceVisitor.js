...
exports.BranchDistanceVisitor = void 0;
...
class BranchDistanceVisitor extends ast_visitor_javascript_1.AbstractSyntaxTreeVisitor {
    constructor(syntaxForgiving, stringAlphabet, variables, inverted) {
        super("", syntaxForgiving);
        this._K = 1; // punishment factor
        this.Statement = (path) => {
            let id;
            ...
            if (this._isDistanceMap.get(id)) {
                _distance = this._valueMap.get(id);
            }
            else {
                if (this._inverted) {
                ...
                if (property.isIdentifier()) {
                    ...
                    switch (property.node.name) {
                        case "endsWith": {
                            ...
                            argument.visit();
                            const argumentValue = (this._valueMap.get(argument.toString()));
                            const endOfObject = objectValue.length > argumentValue.length
                            ...
                            if (this._inverted) {
                                ...
                                if (endOfObject === argumentValue) {
                                    ...
                                    this._valueMap.set(path.toString(), this._normalize(this._realCodedEditDistance(endOfObject, argumentValue)));
                                }
                            }
                            break;
                        }
                        case "startsWith": {
                            ...
                            argument.visit();
                            const argumentValue = (this._valueMap.get(argument.toString()));
                            const startOfObject = objectValue.length > argumentValue.length
                            ...
                            if (this._inverted) {
                                ...
                                if (startOfObject === argumentValue) {
                                    ...
                                    this._valueMap.set(path.toString(), this._normalize(this._realCodedEditDistance(startOfObject, argumentValue)));
                                }
                            }
                            break;
                        }
                            ...
                            if (this._inverted) {
                                ...
                                if (objectValue.includes(argumentValue)) {
                                    ...
                                    if (objectValue.length > argumentValue.length) {
                                        ...
                                        for (let start = 0; start < objectValue.length - argumentValue.length; start++) {
                                            const substring = objectValue.slice(start, argumentValue.length);
                                            minValue = Math.min(minValue, this._realCodedEditDistance(substring, argumentValue));
                                        }
                                        this._valueMap.set(path.toString(), this._normalize(minValue));
                                    }
                                    else {
                                        this._valueMap.set(path.toString(), this._normalize(this._realCodedEditDistance(objectValue, argumentValue)));
                                    }
                                }
                            }
                            break;
        ...
        this.BinaryExpression = (path) => {
            ...
            left.visit();
            right.visit();
            const leftValue = this._valueMap.get(left.toString());
            const rightValue = this._valueMap.get(right.toString());
        ...
        this.LogicalExpression = (path) => {
            ...
            left.visit();
            right.visit();
            let leftValue = this._valueMap.get(left.toString());
            let rightValue = this._valueMap.get(right.toString());
            if (!this._isDistanceMap.get(left.toString())) {
        ...
        BranchDistanceVisitor.LOGGER = (0, logging_1.getLogger)("BranchDistanceVisitor");
    ...
    _getDistance(condition) {
        if (!this._valueMap.has(condition) || !this._isDistanceMap.get(condition)) {
            // the value does not exist or is not a distance
            throw new Error((0, search_1.shouldNeverHappen)("BranchDistanceVisitor"));
        }
        return this._valueMap.get(condition);
    }
    minimum(a, b, c) {
        let mi;
        mi = a;
        ...
        for (index = 1; index <= n; index++) {
            ...
            for (index_ = 1; index_ <= m; index_++) {
                ...
                if (s_index == t_index) {
                    ...
                    //
                    if (!this._stringAlphabet.includes(t_index) ||
                        !this._stringAlphabet.includes(s_index)) {
                        BranchDistanceVisitor.LOGGER.warn(`Cannot search for character missing from the sampling alphabet one of these is missing: ${t_index}, ${s_index}`);
                        cost = Number.MAX_VALUE;
                    }
                    else {
                        cost = Math.abs(this._stringAlphabet.indexOf(s_index) -
                            this._stringAlphabet.indexOf(t_index));
                    }
                    cost = this._normalize(cost);
                }
                // Step 6
                d[index][index_] = this.minimum(d[index - 1][index_] + 1, d[index][index_ - 1] + 1, d[index - 1][index_ - 1] + cost);
            }
        }
        // Step 7
        return d[n][m];
    }
    _editDistDP(string1, string2) {
        ...
        for (let index = 0; index <= m; index++) {
            for (let index_ = 0; index_ <= n; index_++) {
                if (index == 0) {
                    ...
                    table[index][index_] =
                        1 +
                            Math.min(table[index][index_ - 1], Math.min(table[index - 1][index_], table[index - 1][index_ - 1]));
                }
            }
        }
        return table[m][n];
...
exports.BranchDistanceVisitor = BranchDistanceVisitor;
//# sourceMappingURL=BranchDistanceVisitor.js.map