...
const VisitState_1 = require("./VisitState");
...
class Visitor {
    constructor(types, sourceFilePath = "unknown.js", opts = {}) {
        ...
        this.visitState = new VisitState_1.VisitState(types, sourceFilePath, opts.inputSourceMap, opts.ignoreClassMethods, opts.reportLogic);
    }
    enter(path) {
    ...
    exit(path) {
        ...
        const hash = (0, crypto_1.createHash)(SHA)
        ...
        if (this.opts.coverageGlobalScopeFunc) {
            if (path.scope.getBinding("Function")) {
                gvTemplate = globalTemplateAlteredFunction({
                    GLOBAL_COVERAGE_SCOPE: this.types.stringLiteral("return " + this.opts.coverageGlobalScope),
                });
            }
            else {
        ...
        path.node.body.unshift(this.types.expressionStatement(this.types.callExpression(this.types.identifier(this.visitState.varName), [])));
...
function extractAndReplaceVariablesFromTest(scope, test) {
    const variables = [];
    ...
    test.traverse({
        Identifier: {
            enter: (p) => {
                ...
                if (ast_visitor_javascript_1.globalVariables.has(p.node.name) ||
                    ast_visitor_javascript_1.reservedKeywords.has(p.node.name)) {
                    return;
                }
        ...
        CallExpression: {
            enter: (p) => {
                ...
                p.replaceWith(t.sequenceExpression([
...
function coverIfBranches(path) {
    ...
    if (ignoreIf) {
        ...
        if (path.get("consequent").isBlockStatement()) {
            if (path.get("consequent").has("body")) {
                this.insertBranchCounter(path, path.get("consequent").get("body")[0], branch);
            }
            else {
    ...
    if (ignoreElse) {
        ...
        if (path.get("alternate").isBlockStatement()) {
            if (path.get("alternate").has("body")) {
                this.insertBranchCounter(path, path.get("alternate").get("body")[0], branch);
            }
            else {
    ...
    const metaTracker = this.getBranchMetaTracker(branch, testAsString, variables);
    const identifier = path.scope.generateUidIdentifier("test");
    path.insertBefore(t.variableDeclaration("let", [
        ...variables
            // filter duplicates
            .filter(([source, identifier], index) => index ===
            variables.findIndex(([source2, identifier2]) => identifier === identifier2))
    ...
    test.replaceWith(t.sequenceExpression([
        increment,
...
function coverTernary(path) {
    ...
    path
        ...
        .insertBefore(t.variableDeclaration("let", [
        ...variables
            // filter duplicates
            .filter(([source, identifier], index) => index ===
            variables.findIndex(([source2, identifier2]) => identifier === identifier2))
...
const coverageTemplate = (0, core_1.template)(`
...
const metaTemplate = (0, core_1.template)(`
